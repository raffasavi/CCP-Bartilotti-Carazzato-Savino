...
####################################################################################
    ...
    if t%5 == 0 and t != last_check:
        ...
        value = "Sensor [INSERISCI QUI IL NUMERO] * {} * {} *".format(len(faces), datet)
        ...
        if sec>=16 and sec<=23 or sec>=36 and sec<=43 or sec>=54 and sec<=58:
            nameimage = "frame_sensor[INSERISCI QUI IL NUMERO]_{}.jpg".format(num)
            cv.imwrite(nameimage, img)
            files = {'file': open(nameimage, 'rb')}
            r = post(f'{base_url}/sensors/sensor[INSERISCI QUI IL NUMERO]'
